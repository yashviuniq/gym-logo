...
export function usePermissions() {
  ...
  const fetchPermissions = async () => {
    try {
      ...
      // View-only users can view every admin module (writes blocked server-side)
      else if (user.role === 'view_only') {
        setPermissions(OWNER_PERMISSIONS);
      }
      // Trainers have limited permissions (no finance access)
        ...
        setPermissions({
          ...
          attendance: true,
          announcements: true,
          finance: false, // Trainers cannot access finance
          ...
          inquiries: true // Can access inquiries
        });
      }
