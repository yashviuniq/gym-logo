...
export const generateReceiptPDF = async (receiptData) => {
  const {
    ...
    validityStart,
    validityEnd,
    amount, // This is the Paid Amount
    balanceAmount = 0, // Default to 0 if not provided
    paymentMode,
    receiptNumber,
  ...
  // Divider line - Horizontal inside box
  doc.setDrawColor(...borderColor);
  doc.setLineWidth(0.5);
  doc.line(margin + 4, boxY + 28, pageWidth - margin - 4, boxY + 28);
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("TOTAL AMOUNT:", margin + 8, boxY + 38);
  
  doc.setTextColor(...black);
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text(`₹ ${totalAmount.toLocaleString("en-IN")}`, margin + 8, boxY + 46);

  // Paid Amount (right side, bottom)
  doc.setTextColor(...darkBlue);
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("PAID AMOUNT:", dividerX + 8, boxY + 38);
  
  doc.setTextColor(...black);
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text(`₹ ${Number(amount).toLocaleString("en-IN")}`, dividerX + 8, boxY + 46);

  // Balance Amount
  doc.setTextColor(...darkBlue);
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("BALANCE AMOUNT:", margin + 8, boxY + 62);
  
  doc.setTextColor(Number(balanceAmount) > 0 ? 220 : 0, Number(balanceAmount) > 0 ? 38 : 0, Number(balanceAmount) > 0 ? 38 : 0); // Red if balance > 0
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text(`₹ ${Number(balanceAmount).toLocaleString("en-IN")}`, margin + 8, boxY + 70);

  // Payment Mode
  doc.setTextColor(...darkBlue);
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("PAYMENT MODE:", dividerX + 8, boxY + 62);
  
  doc.setTextColor(...black);
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text((paymentMode || "CASH").toUpperCase(), dividerX + 8, boxY + 70);
